...
def test_app_password_loaded_and_redacted() -> None:
    # A clearly-synthetic fixture value — never a real password (see SECURITY.md).
    secret = "fixture-app-password"
    settings = load_settings(env_file=None, environ={"MONEYTOR_APP_PASSWORD": secret})
    assert settings.app_password is not None
    assert settings.app_password.reveal() == secret
    # Included in the redaction set so it never leaks into logs.
    assert secret in settings.secret_values()
