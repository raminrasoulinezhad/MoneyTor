...
def _yield_fraction(value: Any) -> Decimal | None:
    """Questrade's ``yield`` is an annual percentage (1.23 → 1.23%); make it a
    fraction. None/zero/blank means the broker reports no dividend."""
    if value in (None, 0, "0", ""):
        return None
    return _decimal(value, "symbol.yield") / Decimal("100")
    ...
    def _symbol_info(self, symbol_ids: Iterable[int]) -> dict[int, _SymbolInfo]:
        ...
        for symbol in node.get("symbols", []):
            ...
            info[int(symbol["symbolId"])] = _SymbolInfo(
                ...
                dividend_yield=_yield_fraction(symbol.get("yield")),
            )
        return info
