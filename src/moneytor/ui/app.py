...
def run_app(argv: list[str] | None = None) -> int:
    ...
    # Build the cockpit empty and show it maximized first, so the password gate
    # below can lock the *same* window (a full-screen overlay) rather than a
    # separate dialog. No data is loaded until the app is unlocked.
    window = MainWindow(
        people=(),
        provider=provider,
    ...
    def load_data() -> None:
        ...
        if cached is not None:
            window.set_people(apply_sector_map(cached.people, sector_map))
        elif not has_credentials:
            window.set_people(apply_sector_map(demo_people(), sector_map))
    ...
    # Gate the cockpit behind the configured password (MONEYTOR_APP_PASSWORD) as a
    # full-window lock overlay; data loads only once unlocked. Unset = no gate.
    if settings.app_password is not None:
        overlay = window.lock(settings.app_password.reveal())
        overlay.unlocked.connect(load_data)
        overlay.cancelled.connect(window.close)
    else:
        load_data()
